...
class EditImageController extends ApiBaseController
{
    public function __invoke(MediaFileService $service, EditImageRequest $request)
    {
        ...
        $originalFile = MediaFile::find($request->input('id'));
        if (($originalFile && $originalFile->id === $request->input('parent_id')) || $version === 'original') {
            ...
            return new MediaFileResource($originalFile);
        }
