...
class EditImageController extends ApiBaseController
{
    public function __invoke(MediaFileService $service, EditImageRequest $request)
    {
        ...
        if (!$imageWebP) {
            return $this->response(['message' => 'FileVersion not found'], Response::HTTP_BAD_REQUEST);
        }
        ...
        if ($oldUrl) {
            RenameFile::dispatch($oldUrl, $mediaFile->url, $mediaFile->alt_text, $mediaFile->link)
