...
class FeedController extends Controller
{
    ...
    public function generateSitemap()
    {
        ...
        foreach ($slugs as $slug) {
            if (!$slug->slugable) {
                continue;
            } elseif (!in_array(get_class($slug->slugable), $nonAvoidModels)) {
                continue;
            } elseif (get_class($slug->slugable) == 'App\Model\Event' && (!$slug->slugable->category->first() || $slug->slugable->status != 0)) {
                continue;
            }
