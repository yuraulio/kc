...
class FrontendHelperLib
{
    ...
    */
    ...
    /*
     * Some route prefixes can take additional arguments
     * we need to handle these (ie dont take them into account on our slug comparisons)
     */
    public function handleRoutePrefix($str = null)
    {
        if (is_null($str)) {
            return $str;
        } else {
    ...
    public function pRoute($slug = null, $lang = null, $website = null, $optype = "def")
    {
        ...
        return $this->handleRoutePrefix($res);
    }
    ...
    public function splashTpl($content = array())
    {
        ...
        if (!empty($content['featured'])) {
            ...
            $html .= '<img src="'.cdn('/uploads/originals/'.$image['path'].'/'.$image['name'].$image['ext']).'" alt="'.$content['title'].'" />';
        }
        $html .= '</a>';
        $html .= '</div>';
        $html .= '</div>';
    ...
    */
     ...
     */
    public function fetchBanner($abbr = '', $lang = '', $take = "any", $adata = array(), $sort = 'priority')
    {
        $res = [];
        $banners = Content::select('id', 'status', 'title','abbr','priority','ext_url','target','caption','subtitle')->with('featured.media')->filterContent([
            "abbr" => $abbr,
            "lang" => $lang,
            "take" => $take,
            "status" => 1,
        ...
        if ($banners) {
            ...
            foreach ($banners as $key => $row) {
                ...
                if (isset($row['featured']) && $row['featured']) {
                    ...
                    $res[$key]['image'] = cdn('/banner-img').'/'.$row['abbr'].'/'.$media['path'].'/'.$media['name'].$media['ext'].$align_suffix;
                }
    ...
    /*
     * Decide if a given slug or translation group is currently viewed
     * @return a string usually active (a selected class) used in conjuction with css
     * to mark a url as active
     */
    public function activeUrl($groupOrSLug = null, $status = "active", $currentUrl = null)
    {
        $status = ($status ? $status : "active");
        if (is_null($currentUrl)) {
            $segment = Request::segment(1);
            $pRoute = $this->pRoute($groupOrSLug, $_ENV['LANG'], $_ENV['WEBSITE'], 'get');
        } else {
            $pRoute = $groupOrSLug;
            $segment = $currentUrl;
        }
