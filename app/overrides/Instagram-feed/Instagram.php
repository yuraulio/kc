...
class Instagram
{
    ...
    // const STORIES_URL_FORMAT = "https://graph.facebook.com/v15.0/stories?fields=%s&limit=%s&access_token=%s";

    const STORIES_URL_FORMAT = "https://graph.facebook.com/v15.0/%s/stories?fields=%s&access_token=%s";
    const STORIES_FIELDS = 'id,media_type,media_url,thumbnail_url,permalink,timestamp';
    const MEDIA_URL_FORMAT1 = "https://graph.facebook.com/v15.0/%s/media?fields=%s&access_token=%s";
    const MEDIA_FIELDS1 = "caption,id,media_type,media_url,thumbnail_url,permalink,children{media_type,media_url},timestamp,like_count";
    ...
    public function fetchStories($limit = 20)
    {
        $url = sprintf(
        ...
        while ($this->shouldFetchNextPage($response, $collection->count(), $limit)) {
            ...
            $collection = $collection->merge($response['data'])->reject(function ($media) {
                return $this->ignoreVideo($media);
            });
        }
     ...
     */
    public function fetchMedia($limit = 20)
    {
        // $url = sprintf(
