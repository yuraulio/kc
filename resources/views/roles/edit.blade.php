                        ...
                        <form method="post" action="{{ route('role.update', $role) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('permissions') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-permissions">{{ __('Permission') }}</label>
                                    <textarea name="permissions" id="input-permissions" cols="30" rows="10" class="form-control{{ $errors->has('permissions') ? ' is-invalid' : '' }}" placeholder="{{ __('Permissions') }}">{{ old('permissions', $role->permissions) }}</textarea>
