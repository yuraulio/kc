    ...
    <div class="row">
      ...
      <div class="col-md-6 col-xl-6 selection-order ">
        ...
        <div class="participant-full-wrap">
          @if(!Auth::check())<p class="login-link info">Already have an account? <a href="#" class="link-color">Log
              in</a> to retrieve your data. Fields marked with an asterisk (*) are required.</p>@endif
          <form action="{{route('registration')}}" method="post" id="participant-form" name="participant-form">
            ...
            <div class="form-wrp box" @if($i==1) id="clone-box" @else id="clone-box-{{$i}}" @endif>
              ...
              @if(Auth::check())<p class="validation-info">Fields marked with an asterisk <span
                  class="checkout-required-data">(*)</span> are required.</p>@endif

              <div class="form-row">
                <div class="col-md-6 mb-4 pr-md-3">
                  <label class="input-label">My first name is <span class="checkout-required-data">*</span></label>
                  <input type="text" name="firstname[{{$i-1}}]" class="form-control with-focus-visible"
                         value="{{old('firstname', $firstname[$i-1])}}" required>
                  <div class="valid-feedback"></div>
                </div>
                <div class="col-md-6 mb-4 pl-md-3">
                  <label class="input-label">My last name is <span class="checkout-required-data">*</span></label>
                  <input type="text" name="lastname[{{$i - 1}}]" class="form-control"
                         value="{{old('lastname', $lastname[$i-1])}}" required>
                  <div class="valid-feedback"></div>
                </div>
              </div>
              <div class="form-row">
              ...
              <div class="form-row">
                <div class="col-md-12 mb-4 position-relative country-dropdown">
                  <label class="input-label">My mobile phone number is <span
                      class="checkout-required-data">*</span></label>
                  <select name="country_code[{{$i - 1}}]" onchange="selectCountryCode(this,'{{$i}}')"
                          class="form-control country{{$i}}" id="country">
                  ...
                  <input class="required" id="mobile{{$i}}" onkeyup="checkPhoneNumber(this,'{{$i}}')" type="number"
                         name="mobile[{{$i-1}}]" value="{{old('mobile',$mobile[$i-1])}}" />
                  <label id="mobile-error{{$i}}" style="display:none" class="error error-mobile" for="mobile"></label>
                  <input type="hidden" name="mobileCheck[{{$i - 1}}]" id="mobileCheck{{$i}}"
                         value="{{old('mobile',$mobile[$i-1])}}">
                </div>
              </div>
              <div class="form-row">
                <div class="col-md-12 mb-4">
                  <label class="input-label">The town or city I live in is</label>
                  <input type="text" name="city[{{$i - 1}}]" class="form-control" value="{{old('city',$city[$i-1])}}"
              ...
              <div class="form-row">
                <div class="col-md-12 mb-4">
                  ...
                  <input name="company[{{$i - 1}}]" type="text" class="form-control"
                         value="{{old('company',$company[$i - 1])}}" aria-describedby="inputGroupPrepend3">
                </div>
              </div>
              <div class="form-row">
                <div class="col-md-12 mb-4">
                  <label class="input-label">My occupation or title is</label>
                  <input type="text" name="jobtitle[{{$i - 1}}]" value="{{old('jobtitle',$job_title[$i - 1])}}"
                         class="form-control" placeholder="E.g. Marketing specialist"
                         aria-describedby="inputGroupPrepend3">
                </div>
              </div>
              @if($i>1)
              <div class="remove-participant" data-participant-number="{{ $totalitems }}">
                <a href="javascript:void(0)">Remove participant <img src="{{cdn('new_cart/images/close-green.svg')}}"
                                                                     width="9px" height="10px" class="without-hover">
                  <img src="{{cdn('new_cart/images/close-green2.svg')}}" width="9px" height="10px"
                       class="with-hover"></a>
              </div>
              @endif
            </div>
            ...
            <div class="form-row">
              <div class="custom-control custom-checkbox">
                ...
                <p>I have read, agree upon & accept the <a href="/terms" class="link-color">terms & conditions</a> and
                  <a href="/data-privacy-policy" class="link-color">data privacy policy.</a></p>--}}
                <label class="custom-control-label" for="customCheck1"></label>
                <p>I accept the <a href="/terms" target="_blank" class="link-color">Terms & Conditions</a> and I confirm
                  that I have read the <a target="_blank" href="/data-privacy-policy" class="link-color">Data Privacy
                    Policy.</a></p>
              ...
              <div id="customCheck2New" class="custom-control custom-checkbox">
                ...
                <p>Ι accept that by accessing this course, I can't withdraw from payment.</p>
            ...
            <div class="add-participant-wrap d-flex justify-content-between">
              <button class="add-participant" data-participant-number="{{ $totalitems }}" type="button">
                <img src="{{cdn('new_cart/images/plus-full-green.svg')}}" class="without-hover" width="44px"
                     height="44px">
                <img src="{{cdn('new_cart/images/plus-full-green2.svg')}}" class="with-hover" width="44px"
                     height="44px"> <span>Add another participant</span>
              </button>
            </div>
            ...
            <div class="checkout-btn-wrap form-row mb-4 my-md-5 align-items-center">
              <button id="btn" type="submit" class="btn registration checkout-button-secondary">Next: Billing <img
                  src="{{cdn('new_cart/images/arrow-next-red.svg')}}" width="20px" height="12px" class="without-hover"
                  alt=""> <img src="{{cdn('new_cart/images/arrow-next-red2.svg')}}" width="20px" height="12px"
                               class="with-hover" alt=""></button>
            </div>
          </form>
        </div>
      </div>
...
<script>
  $(document).ready(function() {
    dataLayer.push({
      'Event_ID': '{{$tigran['Event_ID'].'p'}}',
      'event': 'Add To Cart',
      'Product_id': '{{$tigran['Product_id']}}',
      'Price': '{{$tigran['Price']}}',
      'ProductCategory': '{{$tigran['ProductCategory']}}',
    });
  });
</script>
