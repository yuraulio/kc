            ...
            <div class="row">
                <div class="col-lg-9 col-md-8 col-sm-7 col-xs-12">
                    ...
                    <div class="posts-category-sections">
                        <div class="home-category-heading home-category-heading-generic section-heading-{{ $cat_dets->id }}">
                            <div class="categ-text">
                                @if ($cat_dets->type == 0)
                                <img alt="{{ $cat_dets->name }}" src="{{ $frontHelp->icon2x($cat_dets['primary_image']) }}" /> {{ $cat_dets->name }}
                                @else
                                <i class="fa fa-hashtag"></i> {{ $cat_dets->name }}
                                @endif
                            </div>
                        </div>

                        <div class="row" id="appendNewContent" data-dp-take="{{ $take }}" data-dp-total="{{ $cat_dets->total }}" data-dp-skip="{{ $list->count() }}" data-dp-section-id="{{ $cat_dets->id }}">
                        @if (!empty($list))
                            @foreach ($list as $key => $row)
                                ...
                                @endif
                                <div class="col-lg-4 col-md-6 col-sm-12 dpContentEntry" data-dp-content-id="{{ $row->id }}">
                                    <div class="home-small-photo-tile">
                                        ...
                                        <h2 class="home-small-post-sub-title">
                                            <a class="section-heading-{{ $cat_dets->id }}>" href="{{ $frontHelp->pSlug($row) }}" title="{{ $frontHelp->pField($row, 'title') }}" target="{{ $frontHelp->pTarget($row) }}">
