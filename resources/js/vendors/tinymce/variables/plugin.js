...
export const Variable = (editor) => {
    ...
    editor.plugins.variable = {
        addVariable: addVariable
    }

    const validList = editor.getParam("variable_valid", null);

    if (validList?.length) {
        editor.ui.registry.addMenuButton('variable', {
            text: 'Variables',
            fetch: function (callback) {
                callback(validList.map(x => {
                    return ({
                        type: 'menuitem',
                        text: x,
                        onAction: function () {
...
if (window?.tinymce?.PluginManager) {
    window?.tinymce.PluginManager.add('variable', Variable);
}
