    ...
    <div>
            ...
            <div v-if="item" class="card">
                ...
                <div class="row mt-3">
                    <div class="col-12 text-center">
                        ...
                        >
                            <i v-if="!loadingModal" class="fe-check-circle me-1"></i
                            ><i v-else class="fas fa-spinner fa-spin"></i> Create
                        </button>
                        <button
                            v-if="type == 'edit'"
                            :disabled="loadingModal"
                            ...
                            class="btn btn-soft-success waves-effect waves-light m-1"
                        >
                            <i v-if="!loadingModal" class="mdi mdi-square-edit-outline me-1"></i
                            ><i v-else class="fas fa-spinner fa-spin"></i> Save
                        </button>
                        <button
                            @click="$modal.hide('edit-modal')"
                            type="button"
                            class="btn btn-soft-secondary waves-effect waves-light m-1"
                        >
        ...
        <b-sidebar id="sidebar-right" title="" right shadow>
            ...
            <template v-else>
                ...
                <div class="px-3 py-1">
                    <button @click="refreshTable()" class="btn btn-soft-info waves-effect waves-light">
                        Refresh Table
                    </button>
                </div>
            </template>
                ...
                <div v-if="widgets[0]" class="col-lg-3 col-md-6">
                    <div class="card bg-pattern mb-3">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <h4 class="text-dark my-1 text-truncate" :title="widgets[0]">
                                        <span>{{ widgets[0] }}</span>
                                    </h4>
                                </div>
                                <div class="col-12">
                                    <div class="text-start">
                                        <p class="text-muted mb-0 text-truncate">
                                            PUBLISHED: {{ widgets[1]['published'] }},
                                            <span>UNPUBLISHED: {{ widgets[1]['unpublished'] }}</span>
                                        </p>
                                    </div>
                                </div>
                                <div class="col-12">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-12">
                                    <div class="text-start">
                                        <p class="text-muted mb-0 text-truncate">
                                            Total: € {{ widgets[1][0] !== undefined ? widgets[1][0] : 0 }}
                                        </p>
                                    </div>
                                </div>
                                <div class="col-12">
        ...
        <div class="card mb-2">
            <div class="card-body pt-0" style="overflow: auto">
                <div class="row mb-1">
                    <div
                        v-if="
                            !config.apiUrl.includes('royalties') ||
                            (config.apiUrl.includes('royalties') && config.royaltyView != 'single')
                        "
                        class="col-md-auto pt-3 d-flex align-items-center"
                    >
                        <input
                            v-model="filter"
                            type="search"
                            class="form-control my-1 my-md-0 d-inline-block w-auto"
                            id="inputPassword2"
                            placeholder="Search..."
                    ...
                    <div class="col-md-auto pt-3 d-flex align-items-center">
                        <div>
                            <span
                                @click="(page_value = null), refreshTable()"
                                v-if="page_value && showFilter('page')"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >{{ page_value.title.substring(0, 20) }} <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <span
                                @click="(dynamic = null), refreshTable()"
                                v-if="dynamic && showFilter('dynamic')"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >{{ dynamic.title }} <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <span
                                @click="(published_value = null), refreshTable()"
                                v-if="published_value && showFilter('visibility')"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >{{ published_value.title }} <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <span
                                @click="(template_value = null), refreshTable()"
                                v-if="template_value && showFilter('template')"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >{{ template_value.title }} <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <template v-for="(pageType, key) in type_value">
                                <span
                                    @click="type_value.splice(key, 1), refreshTable()"
                                    v-if="pageType && showFilter('type')"
                                    class="badge bg-primary ms-1 cursor-pointer"
                                    >{{ pageType.title }} <i class="fa fa-times" aria-hidden="true"></i
                                ></span>
                            </template>
                            <span
                                @click="(category_value = null), refreshTable()"
                                v-if="category_value && showFilter('category')"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >{{ category_value.title }} <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <span
                                @click="(subcategory_value = null), refreshTable()"
                                v-if="subcategory_value && showFilter('subcategory')"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >{{ subcategory_value.title }} <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <span
                                @click="!loading ? ((transaction_from = null), refreshTable()) : {}"
                                v-if="transaction_from"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >Transaction From: {{ formatdate(transaction_from) }}
                                <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                            <span
                                @click="!loading ? ((transaction_to = null), refreshTable()) : {}"
                                v-if="transaction_to"
                                class="badge bg-primary ms-1 cursor-pointer"
                                >Transaction To: {{ formatdate(transaction_to) }}
                                <i class="fa fa-times" aria-hidden="true"></i
                            ></span>
                        </div>
                    </div>
                    ...
                    <div class="col-md align-items-center">
                        ...
                        <div
                            ...
                            class="btn-group dropleft multiselect-actions float-end ms-2 pt-3"
                            role="group"
                        >
                            <button
                        ...
                        <div v-show="!config.apiUrl.includes('royalties')">
                            <div class="btn-group dropleft multiselect-actions float-end ms-2 pt-3">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2">
                                    <a class="dropdown-item" href="#" @click="changePerPage(10)"> 10 </a>
                                    <a class="dropdown-item" href="#" @click="changePerPage(50)"> 50 </a>
                                    <a class="dropdown-item" href="#" @click="changePerPage(100)"> 100 </a>
                                    <a class="dropdown-item" href="#" @click="changePerPage(0)"> All </a>
                                </div>
                            </div>
                        </div>

                        <div
                            v-show="config.apiUrl.includes('royalties')"
                            class="btn-group dropleft multiselect-actions float-end pt-3"
                        >
                    ...
                    <template slot="royalties_title" slot-scope="props">
                        <a
                            :href="
                                ...
                                props.rowData.id +
                                '?transaction_from=' +
                                formatDate(transaction_from) +
                                '&transaction_to=' +
                                formatDate(transaction_to)
                            "
                    ...
                    <template slot="visibility" slot-scope="props">
                        ...
                        <template v-if="props.rowData.dynamic !== undefined">
                            <template v-if="props.rowData.dynamic == 0">
                                ...
                                >
                                    <input
                                        :key="props.rowData.id + 'on'"
                                        @click="changePublish(props.rowData)"
                                        :id="props.rowData.id + 'input'"
                                        type="checkbox"
                                        class="form-check-input"
                                        name="color-scheme-mode"
                                        value="light"
                                        :for="props.rowData.id + 'input'"
                                        :checked="props.rowData.published"
                                    />
                                </div>
                            </template>
                            <template v-else>
                        ...
                        <template v-else>
                            ...
                            >
                                <input
                                    :key="props.rowData.id + 'on'"
                                    @click="changePublish(props.rowData, props.rowField.model)"
                                    :id="props.rowData.id + 'input'"
                                    type="checkbox"
                                    class="form-check-input"
                                    name="color-scheme-mode"
...
export default {
    ...
    methods: {
        ...
        add() {
            ...
            axios[this.type == 'edit' ? 'put' : 'post'](
                this.config.apiUrl + (this.type == 'edit' ? '/' + this.item.id : ''),
                this.item
            )
                .then((response) => {
                            ...
                            if (response.status == 200) {
                                ...
                                if (this.$refs.vuetable.tableData.length == 0) {
                                    if (this.$refs.vuetable.tablePagination.current_page > 1) {
                                        this.$refs.vuetable.changePage(
                                            this.$refs.vuetable.tablePagination.current_page - 1
                                        );
                                    }
                                }
                            }
        ...
        selectAllItems() {
            ...
            var checkbox = document
                .getElementsByClassName('vuetable-th-component-checkbox')[0]
        ...
        deleteMultipleItems() {
            ...
            if (selected.length > 0) {
                Swal.fire({
                    title: 'Are you sure?',
                    text:
                        "You won't be able to revert this! Delete all " +
                    ...
                    if (result.isConfirmed) {
                        Swal.fire(
                            'Delete started!',
                            'Items are being deleted in the background. This will take a few minutes.',
                            'success'
                        );
                    }
                });
        ...
        fixError(message, subcategoryError, subcategory) {
            return (
                message.replace(subcategoryError, " subcategory '" + subcategory + "' ") +
