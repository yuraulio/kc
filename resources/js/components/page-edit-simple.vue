  ...
  <div>
    <div class="page-edit-simple" v-if="page && page.template">
          ...
          <div class="page-title-box mt-3 row">
            ...
            <div class="col-md-6">
              <button
                ...
                @click="changeMode()"
                type="button"
                class="btn btn-soft-info waves-effect waves-light float-end ms-2 mb-3"
              >
                <i class="dripicons-toggles me-1" style="transform: translateY(2px)"></i>Page builder
              </button>
              <button
                :disabled="loading"
                @click="type == 'new' ? add() : edit()"
                type="button"
                class="btn btn-soft-success waves-effect waves-light float-end ms-2 mb-3"
              >
                <i v-if="!loading" class="mdi mdi-square-edit-outline me-1"></i
              ...
              <template v-if="type != 'new'">
                <button
                  ...
                  @click="preview()"
                  type="button"
                  class="btn btn-soft-warning waves-effect waves-light float-end ms-2 mb-3"
                >
                  <i class="dripicons-preview me-1" style="transform: translateY(2px)"></i>Preview
                </button>
                ...
                <template v-if="!page.dynamic">
                  <template v-if="page.published">
                    <input
                      ...
                      type="checkbox"
                      class="btn-check float-end"
                      id="btn-check-outlined"
                      autocomplete="off"
                      :checked="page.published"
                    />
                    <label class="btn btn-outline-primary float-end ms-2 mb-3" for="btn-check-outlined"
                      ><i class="dripicons-checkmark me-1" style="transform: translateY(2px)"></i>Published</label
                    >
                  </template>
                  <template v-else>
                    <input
                    ...
                    />
                    <label class="btn btn-outline-primary float-end ms-2 mb-3" for="btn-check-outlined"
                      ><i class="dripicons-cross me-1" style="transform: translateY(2px)"></i> Unpublished</label
                    >
                  </template>
                </template>
              </template>
                  ...
                  <div class="card-body">
                    <div :class="'row component-tabs ' + (column.tab == 'settings' ? 'settings' : 'main')">
                      <div class="col-12">
                        ...
                        <i
                          v-if="column.template.removable !== false"
                          @click="removeRow(row_index)"
                          class="dripicons-trash text-muted float-end ms-2 cursor-pointer"
                          title="Delete component"
                        ></i>
                        <i
                          v-if="settingsExist(column)"
                          @click="column.tab == 'settings' ? (column.tab = 'main') : (column.tab = 'settings')"
                          :class="
                            'settings-icon text-muted float-end ms-2 ' +
                            (column.template.simple_view_settings_icon
                              ? column.template.simple_view_settings_icon
                              : 'dripicons-gear')
                          "
                        ></i>
                        <template v-if="simpleColumnCount(row.columns) > 1">
                          <ul
                            :class="
                              'nav column-navigation d-inline-block float-end mb-0 nav-row' +
                              row_index +
                              ' ' +
                              (settingsExist(column) == false ? 'column-navigation-margin' : '')
                            "
                          >
                            <template v-for="(column, column_index) in row.columns">
                              <li :class="'nav-item d-inline-block nav-column' + column_index">
                                <a
                                  @click="showColumnTab(row_index, column_index)"
                                  :class="'nav-link text-muted ' + (column_index == 0 ? 'active' : '')"
                                  href="#!"
                                  >Col {{ column_index + 1 }}</a
                                >
                              </li>
                            </template>
                          </ul>
      ...
      <div v-if="isBasicEditorEmpty() && tab != 'Meta'" class="text-center mt-3">
        <p>There are no basic component to edit. Add a new basic component or switch to Page builder.</p>
        <button
          :disabled="loading"
          @click="changeMode()"
          type="button"
      ...
      <component-modal :row="null" :column="null" name="simple"></component-modal>
    </div>
    ...
    preview() {
      ...
      axios
        ...
        .then((response) => {
          if (response.status == 200) {
            this.loading = false;
            window.open(process.env.MIX_APP_URL + '/__preview/' + this.page.uuid + '?p=HEW7M9hd8xY2gkRk', '_blank');
          }
        })
        .catch((error) => {
          console.log(error);
