<template>
  <div>
    <div v-if="!pseudo">
      ...
      <div v-if="name == 'main'" class="row mb-1">
        <div class="col-12">
          <ul class="nav nav-pills nav-fill">
            <li class="nav-item">
              <a @click="tab = 'Content'" :class="'nav-link ' + (tab == 'Content' ? 'active' : '')" href="#!"
                >Content</a
              >
            </li>
            <li class="nav-item">
        ...
        <div class="col-12">
          <nav class="nav nav-pills flex-column flex-sm-row navtab-bg">
            <template v-for="(tab_item, tab_index) in tabs">
              <a
                @click="tabs_tab = tab_item"
                :class="'mb-1 me-1 ms-0 flex-sm-fill text-sm-center nav-link ' + (tabs_tab == tab_item ? 'active' : '')"
                href="#!"
              >
                ...
                <template v-else>
                  ...
                  <div
                    @click="tab_open_edit.splice(tab_open_edit.indexOf(tab_item), 1)"
                    class="d-inline-block float-end ms-1"
                  >
            ...
            <button @click="add_tab = true" class="mb-1 btn btn-soft-success add-column-button">Add tab</button>
          </nav>
        </div>
      ...
      <draggable v-if="data" v-model="data" key="cols" handle=".handle">
        <transition-group tag="div">
          <div
            v-for="(val, index, key) in data"
            :key="'prim' + index"
            :class="
              'col-12 mb-1 card toggle-card ' +
              (name == 'tabs' && tabs_tab == val.tabs_tab ? ' tab-components d-block ' : ' d-none ') +
              (name == 'main' && tab == val.tab ? ' d-block ' : ' d-none ')
            "
          >
            <div class="row handle">
              <div class="col-2">
                <button
                  @click="toggleCollapse(val)"
                  class="btn btn-sm btn-soft-secondary"
                  type="button"
                  data-bs-toggle="collapse"
                  :data-bs-target="'#collapseelement' + val.id"
                  aria-expanded="false"
              ...
              <div class="col-4 text-center">
                <div v-if="val.width" class="btn-group">
                  ...
                  <div class="dropdown-menu dropdown-menu-center" style="" data-popper-placement="bottom-start">
                    <a
                      @click.prevent="val.width = 'full'"
                      :class="'dropdown-item ' + (val.width == 'full' ? 'active' : '')"
                      href="#!"
                      >Full Width</a
                    >
                    <a
                      @click.prevent="val.width = 'content'"
                      :class="'dropdown-item ' + (val.width == 'content' ? 'active' : '')"
                      href="#!"
                      >Content Width</a
                    >
                    <a
                      @click.prevent="val.width = 'blog'"
                      :class="'dropdown-item ' + (val.width == 'blog' ? 'active' : '')"
                      href="#!"
                      >Blog Width</a
                    >
                  </div>
              ...
              <div class="col-4 text-center">
                <div v-if="val.disable_color !== true" class="btn-group">
                  <button
                    ...
                    aria-expanded="true"
                  >
                    <template v-if="val.color == 'white'"> White <i class="mdi mdi-chevron-down"></i> </template>
                    <template v-if="val.color == 'blue_gradient'">
                      Blue gradient <i class="mdi mdi-chevron-down"></i>
                    </template>
                    <template v-if="val.color == 'blue'"> Blue <i class="mdi mdi-chevron-down"></i> </template>
                    <template v-if="val.color == 'gray'"> Gray <i class="mdi mdi-chevron-down"></i> </template>
                  </button>
                  <div class="dropdown-menu dropdown-menu-center" style="">
                    <a
                      @click.prevent="val.color = 'white'"
                    ...
                    <a
                      @click.prevent="val.color = 'blue_gradient'"
            ...
            <div v-for="(column, indr, key) in val.columns" :key="key">
              <span
                ...
                v-if="indr == val.columns.length - 1 && val.removable !== false"
                class="position-absolute top-0 start-100 close-button"
              ...
              <div :key="activeChange" v-show="column.template && column.active" class="">
                <div class="card-body pb-3">
                  <div v-if="val.columns.length > 1">
                    <ul class="nav nav-pills navtab-bg nav-justified">
                      <li
                        v-for="(v, ind) in val.columns"
                        :key="ind"
                        :class="'nav-item ' + getColumnWidth(column, val.columns)"
                      >
                        <a
                          href="#home1"
                          @click="setTabActive(index, ind)"
                          data-bs-toggle="tab"
                          aria-expanded="false"
                          :class="'nav-link' + (v.active === true ? ' active' : '')"
                        >
                          {{ v.template.title }}
                          <div
                            @click.stop="removeColumn(val.columns, column.id, index, ind)"
                            class="d-inline-block float-end ms-1"
                          >
                        ...
                        <div v-if="val.columns.length > 1" class="mt-1">
                          Width: {{ v.width }} / 6
                          <input
                            @click="calculateWidth(val.columns, ind, $event, index)"
                            :value="v.width"
                            class="w-100"
                            type="range"
                            maxlength="1"
                        ...
                        <div v-if="v.template.dynamic != null" class="d-block text-center ms-2">
                          <span class="text-muted font-13 d-inline-block me-1" style="margin-top: 4px">Dynamic</span
                          ><input v-model="v.template.dynamic" type="checkbox" class="form-check-input" />
                        </div>

                        <div class="d-block text-center">
                          <span class="text-muted font-13 d-inline-block me-1" style="margin-top: 4px"
                            >Show on mobile</span
                          ><input v-model="v.template.mobile" type="checkbox" class="form-check-input" />
                        </div>
                        ...
                        <div v-if="checkComponentVersion(v)" class="alert alert-warning m-2" role="alert">
                          ...
                          <i
                            @click="refreshComponent(v)"
                            style="font-size: 20px"
                            class="text-muted dripicons-clockwise m-1 d-inline-block"
                          ></i>
                        </div>
                      </li>
                      <li v-if="val.columns.length < 3">
                        <button
                          @click="split(val.columns, indr, 'push')"
                          class="btn btn-success add-column-button ms-1"
                        >
                  ...
                  <template v-else>
                    <h5 :class="'card-title mb-0 ' + getColumnWidth(column, val.columns)">
                      ...
                      <span
                        v-if="column.template.one_column != true"
                        class="text-muted font-13 float-end"
                      ...
                      <div v-if="column.template.dynamic != null" class="d-inline-block float-end ms-2">
                        <span class="text-muted font-13 d-inline-block me-1" style="margin-top: 4px">Dynamic</span
                        ><input v-model="column.template.dynamic" type="checkbox" class="form-check-input" />
                      </div>

                      <div v-if="column.template.mobile != null" class="d-inline-block float-end">
                        <span class="text-muted font-13 d-inline-block me-1" style="margin-top: 4px"
                          >Show on mobile</span
                        ><input v-model="column.template.mobile" type="checkbox" class="form-check-input" />
                      </div>
                    </h5>
                    <div v-if="checkComponentVersion(column)" class="alert alert-warning m-2" role="alert">
                      This component is updated. Refresh to get new version. Refresh will delete data.
                      <i
                        @click="refreshComponent(column)"
                        style="font-size: 20px"
                        class="text-muted dripicons-clockwise m-1 d-inline-block"
                      ></i>
                    </div>
                ...
                <div
                  :class="'tab-content collapse pb-3 ' + (val.initialCollapsed ? '' : 'show')"
                  :id="'collapseelement' + val.id"
                  style="padding-top: 0px"
                >
                  <div
                    v-for="(vl, indx) in val.columns"
                    :key="'tabpane' + indx"
                    :class="'tab-pane ' + (vl.active === true ? ' active' : '')"
                  >
                      ...
                      <div v-show="!column.template.dynamic" class="card-body row pb-0 pt-0">
                        <div class="col-12">
                          ...
                          <div class="text-center">
                            <iframe
                              :width="
                                findInputValue(column.template.inputs, 'youtube_full_width')
                                  ? '100%'
                                  : findInputValue(column.template.inputs, 'youtube_width') || '100%'
                              "
                              :height="findInputValue(column.template.inputs, 'youtube_height') || '600'"
                              :src="
                                'https://www.youtube.com/embed/' +
                                getYoutubeVideoCode(findInputValue(column.template.inputs, 'youtube_embed'))
                              "
    ...
    <div v-if="data && pseudo">
      <div v-if="name == 'main'" class="row">
        <div class="col-sm-12 mt-3 mb-3">
          <div class="page-title-box row">
            ...
            <div class="col-md-6">
              <a href="/templates" class="btn btn-soft-secondary waves-effect waves-light float-end ms-2"
                ><i class="fe-x me-1"></i> Cancel</a
              >
              <button
                v-if="mode == 'new' || mode == 'edit'"
                @click="emitTemplateSave()"
                type="button"
                class="btn btn-soft-success waves-effect waves-light float-end"
              >
                <i class="fe-check-circle me-1"></i> Save
              </button>
            </div>
          </div>
        </div>
      ...
      <draggable v-model="data" key="cols" handle=".handle">
        <transition-group tag="div">
          <div
            ...
            :class="
              'row mb-1 ' +
              (name == 'tabs' && tabs_tab == val.tabs_tab ? ' d-block ' : ' d-none ') +
              (name == 'main' && tab == val.tab ? ' d-block ' : ' d-none ')
            "
          >
            <div class="text-center">
            ...
            <div
              v-for="(column, indr) in val.columns"
              :key="'column' + indr"
              :class="'d-inline-block col-lg-' + getColumnWidth(column, val.columns)"
            >
              <div class="" style="position: relative">
                <div :key="'pseudo' + indr" class="dropzone mb-2" style="min-height: 150px">
                  <div
                    class="dz-message needsclick"
                    style="margin: 0px !important; display: flex; justify-content: center; flex-direction: column"
                  >
                    ...
                    <span v-if="column.template.one_column != true" class="text-muted font-13">
                      <i
                        @click="split(val.columns, indr, 'push')"
                        v-if="val.columns.length < 3"
                        class="success dripicons-stack mr-2"
                      ></i>
                      <i
                        v-if="val.columns.length <= 3 && val.columns.length > 1 && indr > 0"
                        @click="split(val.columns, indr, 'splice')"
                        class="danger dripicons-trash mr-2"
                      ></i>
                      <br />
                      <template v-if="val.columns.length > 1">
                        Width: {{ column.width }} / 6
                        <input
                          @click.stop="calculateWidth(val.columns, indr, $event, index)"
                          :value="column.width"
                          class="w-100"
                          type="range"
                          maxlength="1"
    ...
    <component-modal :row="row_index" :column="column_index" :name="name"></component-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setTabActive(index, ind) {
      this.data[index].columns.forEach((column) => {
        column.active = false;
      });
  ...
  mounted() {
    ...
    eventHub.$on('component-change-' + this.name, (data) => {
      ...
      this.data[row_index].columns[column_index].template = JSON.parse(
        JSON.stringify(this.extractedComponents[component])
      );
