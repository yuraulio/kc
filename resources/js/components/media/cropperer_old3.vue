...
export default {
    ...
    methods: {
        setupPrevalue(from_function = false) {
            ...
            if (this.version == 'original') {
                ...
                if (this.versionsForUpdate['original']) {
                    this.alttext = this.versionsForUpdate['original'].alttext
                        ? this.versionsForUpdate['original'].alttext
                        : '';
                } else {
            ...
            if (!from_function) {
                ...
                if (!foundVersion) {
                    if (!baseUrl.includes('admin')) {
                        if (
                            this.$parent.$parent.firstLoadedData.id != this.parrentImage.id &&
        ...
        changeHandler(event) {
            ...
            if (this.selectedVersion) {
                this.versionsForUpdate[this.selectedVersion.version][input] =
                    event.target.value !== undefined && event.target.value != null ? event.target.value : '';
            } else {
                this.versionsForUpdate['original'][input] =
                    event.target.value !== undefined && event.target.value != null ? event.target.value : '';
            }
        },
        onMoveCropBox() {
        ...
        setCropBox(image_width, image_height) {
            ...
            if (
                this.versionsForUpdate[this.selectedVersion.version] &&
                this.versionsForUpdate[this.selectedVersion.version].crop_data !== undefined &&
                !this.versionsForUpdate[this.selectedVersion.version].hasDeleted
            ) {
                ...
                this.$set(
                    this.cropBoxData,
                    'left',
                    (container_width - canvas_width) / 2 + width_offset * this.width_ratio
                );
                this.$set(
                    this.cropBoxData,
                    'top',
                    (container_height - canvas_height) / 2 + height_offset * this.width_ratio
                );
            ...
            if (this.uploadedVersions) {
                ...
                if (data != null && data.crop_data == null) {
                    ...
                    this.$set(
                        this.cropBoxData,
                        'left',
                        (container_width - this.selectedVersion.w * this.width_ratio) / 2
                    );
                    this.$set(
                        this.cropBoxData,
                        'top',
                        (container_height - this.selectedVersion.h * this.height_ratio) / 2
                    );
                } else if (data != null) {
        ...
        upload(event) {
            if (
                ...
                this.versionsForUpdate[this.selectedVersion.version] === undefined
            ) {
                this.getCropBoxData();
                this.$refs.cropper
                    .getCroppedCanvas({
