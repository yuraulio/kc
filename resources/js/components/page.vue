...
<template>
    <div class="col-12">
        <div class="card project-box ribbon-box">
            <div class="card-body">
                <div v-if="page.published == true" class="ribbon ribbon-success float-end">
                    <i class="mdi mdi-access-point me-1"></i> Published
                </div>
                <div v-else class="ribbon ribbon-danger float-end">
                    <i class="mdi mdi-access-point me-1"></i> Not Published
                </div>
                    ...
                    <div class="col-11">
                        <h4 class="mt-0">
                            <template v-if="page.type == 'Blog'">
                                <a :href="'/v2/blog/' + page.slug" target="_blank" class="text-dark">{{
                                    page.title
                                }}</a>
                            </template>
                            <template v-else>
                        ...
                        <template v-for="(cat, index) in page.categories">
                            <a :href="'/categories?filter=' + cat.title" :key="index">
                                <div class="badge bg-primary text-white mb-3 mr-2 font-14" style="margin-right: 5px">
                                    {{ cat.title }}
                                </div>
                            </a>
                        </template>
                        <!-- Desc-->
                        <p v-if="page.description" class="text-muted font-13 mb-3 sp-line-2">{{ page.description }}</p>
                        <!-- Task info-->
                        <p class="mb-0 pb-0">
                            ...
                            <span class="pe-2 text-nowrap d-inline-block" style="float: right">
                                <a :href="'/page/' + page.id" class="action-icon">
                                    <i class="mdi mdi-square-edit-outline"></i
                                ></a>
                            </span>

                            <span class="text-sm-start" style="flat: right">
                                <span v-if="page.published_from"
                                    >The page will be published automatically {{ page.published_from }}.
                                </span>
                                <span v-if="page.published_to"
                                    >The page will be unpublished automatically {{ page.published_to }}</span
                                >
                                <div
                                    v-if="!page.published_from && !page.published_to"
                                    :key="page.id"
