<template>
    <div>
                        ...
                        <draggable v-model="data">
                            <transition-group>
                                <div class="row" v-for="(value, ind) in data" :key="ind + 'drag'">
                                    <div
                                        :class="'col-lg-' + 12 / value.columns.length"
        ...
        <form
            @click.prevent="addCustomComponent"
        ...
        >
            <div class="dz-message needsclick" style="margin: 0px !important">
                ...
                <div class="text-center">
                    <span class="text-muted font-13"> <strong>Click to Add Custom Component</strong></span>
                </div>
            </div>
        </form>
    </div>
...
export default {
    ...
    watch: {
        template() {
            ...
            var parsed =
                typeof this.template.rows === 'string' || this.template.rows instanceof String
                    ? JSON.parse(this.template.rows)
                    : this.template.rows;
                ...
                element.columns.forEach((column) => {
                    ...
                    if (this.extractedComponents[column.component] != null) {
                        ...
                        column.template.inputs.forEach((input) => {
                            input.key = input.key + column.order;
                        });
                    }
                });
            });
